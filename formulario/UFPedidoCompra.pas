...
type
  TFPedidoCompra = class(TFPai)
    ...
    edtDataPesqInicial: TJvDateEdit;
    edtDataPesqFinal: TJvDateEdit;
    Label2: TLabel;
    edtValor: TEdit;
    ckApenasNaoEnviados: TCheckBox;
    ...
    ckApenasPedidoNaoImportados: TCheckBox;
    edtSequencial: TDBEdit;
    Label8: TLabel;
    pm1: TPopupMenu;
...
procedure TFPedidoCompra.CriarEstruturaCDS;
begin
...
procedure TFPedidoCompra.dbgrdDocumentoDblClick(Sender: TObject);
begin
  inherited;
  CDSDocumentoImportado.InverterSelecao;
end;

procedure TFPedidoCompra.dbgrdDocumentoDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  inherited;
...
procedure TFPedidoCompra.dbgridConsultaDblClick(Sender: TObject);
begin
  CopiarDadosConsulta;
end;

procedure TFPedidoCompra.edtSequencialExit(Sender: TObject);
begin
  inherited;
  mQuery.Close;
...
procedure TFPedidoCompra.btnBuscarDocumentosClick(Sender: TObject);
begin
  inherited;
...
procedure TFPedidoCompra.btnConsultaClick(Sender: TObject);
begin
  inherited;
...
procedure TFPedidoCompra.btnExcluirClick(Sender: TObject);
var
  objId: TMongoOID;
...
procedure TFPedidoCompra.btnExportarClick(Sender: TObject);
begin
...
procedure TFPedidoCompra.btnGerarPedidoCompraClick(Sender: TObject);
begin
  inherited;
  try
...
procedure TFPedidoCompra.btnVisualizarClick(Sender: TObject);
var
  ST: TMemoryStream;
  Arquivo: string;
  LSArquivo: TStrings;
...
procedure TFPedidoCompra.btnVoltarClick(Sender: TObject);
begin
  inherited;
  IrParaDados;
end;

procedure TFPedidoCompra.btnPesquisarClick(Sender: TObject);
begin
  inherited;
  mQuery.Close;
...
procedure TFPedidoCompra.btnSalvarClick(Sender: TObject);
begin
  inherited;
  try
...
procedure TFPedidoCompra.BuscarDocumentos;
var
  retorno: TJSON;
  Json, JsonTmp, FiltroSituacao: string;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          // Adicionado campo para controle dos pedidos (se gerou compra ou não)
          Json.Insert(1, '"pedidoImportado":"N",');

          mQueryDocumentoImportado.Close;
          mQueryDocumentoImportado.QMatch := '"numero":"' + FuncoesJson.GetValueJson(Json, 'numero') + '"';
          mQueryDocumentoImportado.Open;

          if mQueryDocumentoImportado.IsEmpty then
          begin
            doc.AsJSON := Json;
            conexao[cNomeBancoDados][cColecaoPedido].Insert(doc);
          end;
        end;
      end;
    except
...
procedure TFPedidoCompra.cbCampoPesquisaChange(Sender: TObject);
begin
  inherited;
  edtDataPesqInicial.Visible := (cbCampoPesquisa.ItemIndex = 1);
  edtDataPesqFinal.Visible := (cbCampoPesquisa.ItemIndex = 1);

  edtValor.Visible := (cbCampoPesquisa.ItemIndex = 0);
end;

function TFPedidoCompra.EnviarEmail(Email, Msg: string; Arquivos: TStrings): Boolean;
var
...
procedure TFPedidoCompra.CriarRelacionamentoCDS;
begin
...
procedure TFPedidoCompra.RemoverRelacionamentoCDS;
begin
...
procedure TFPedidoCompra.ValidateItem(Sender: TField);
begin
...
procedure TFPedidoCompra.PopularCDSDocumentoImportado;
var
...
procedure TFPedidoCompra.GerarPedidoCompraPorFornecedor;
var
...
procedure TFPedidoCompra.AtualizarDocumentoEnviado(Codigo: Integer);
var
  Doc: TMongoUpdate;
begin
...
procedure TFPedidoCompra.AtualizarDocumentoImportado(NumeroPedido: string);
var
  Doc: TMongoUpdate;
begin
  Doc := TMongoUpdate.Create(conexao.Env);
...
procedure TFPedidoCompra.InserirDocumento;
var
  I: Integer;
  Doc: TMongoDocument;
begin
...
procedure TFPedidoCompra.AtualizarDocumento(Codigo: Integer);
var
  I: Integer;
  Doc: TMongoUpdate;
begin
...
procedure TFPedidoCompra.IrParaDados;
begin
...
procedure TFPedidoCompra.mniDesmarcartodos1Click(Sender: TObject);
begin
  inherited;
  LockWindowUpdate(Self.Handle);
  try
...
procedure TFPedidoCompra.mniMarcartodos1Click(Sender: TObject);
begin
  inherited;
  LockWindowUpdate(Self.Handle);
  try
...
procedure TFPedidoCompra.mQueryAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TFPedidoCompra.CopiarDadosConsulta;
var
  DSItens: TDataSet;
begin
  inherited;
